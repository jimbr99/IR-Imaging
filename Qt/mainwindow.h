...
int MainWindow::colorMapSel = MAP0;
...
{
    ui->setupUi(this);
    timerId = startTimer(50); // 900
//    QTimer *timer = new QTimer(this);
//   connect(timer, SIGNAL(timeout()), this, SLOT(update()));
//    timer->start(1000);

    this->setWindowTitle("BH-Analytics Lepton Demo");
...
void MainWindow::updateImage(unsigned short *data, int minValue, int maxValue){
    ...
    for (int y = 0; y < LeptonThread::FrameHeight; ++y) {
        for (int x = 0; x < LeptonThread::FrameWidth; ++x) {
            ...
            case MAP1:
                rgbImage.setPixel(x, y, qRgb(colormap1[3*scaledValue], colormap1[3*scaledValue+1], colormap1[3*scaledValue+2]));
            break;
    ...
    if(digitalRead(1) == 0)
    {
        rgbImage.save(QString("/dev/shm/therm.jpg"), "JPG", 100);
        digitalWrite(22, 1); //toggle^=1);
    }
}
...
//    ++snapshotCount;
    ...
    //rgbImage.save(QString("/home/pi/webpy/static/therm.jpg"), "JPG", 100);
 //   rgbImage.save(QString("/dev/shm/therm.jpg"), "JPG", 100);
    if(digitalRead(1))
    {
        digitalWrite(22, 0);  //toggle^=1);
    }
}
...
void MainWindow::saveSnapshot() {
    ...
    // Use USB stick root name (e.g. KINGSTON such as, /media/pi/KINGSTON) to write file to external USB device.
    ...
    // JPG image, top quality
    rgbImage.save(QString("/dev/shm/therm.jpg").arg(snapshotCount), "JPG", 100);
}
